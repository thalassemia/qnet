...
def main(args):
    ...
    def model(number):
        p = pd.read_csv("/u/scratch/s/seanchea/tfchip/Ucobind/down_train.csv", sep = "\t", index_col = 0)
        def shuffle(i):
            row = p.iloc[:, i].values
        ...
        p = pd.read_csv("/u/scratch/s/seanchea/tfchip/Ucobind/down_test.csv", sep = "\t", index_col = 0)
        ...
        saveDir=f'/u/scratch/s/seanchea/u_models_down/{number}/python/'
