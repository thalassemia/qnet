...
def main(args):
    ...
    def model(number):
        p = pd.read_csv("/u/scratch/s/seanchea/tfchip/Lcobind/lisa_train.csv", sep = "\t", index_col = 0)
        def shuffle(i):
            row = p.iloc[:, i].values
        ...
        p = pd.read_csv("/u/scratch/s/seanchea/tfchip/Lcobind/lisa_test.csv", sep = "\t", index_col = 0)
        ...
        saveDir=f'/u/scratch/s/seanchea/lisa_models/{number}/python/'
