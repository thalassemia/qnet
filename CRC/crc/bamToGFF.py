...
def mapBamToGFF(bamFile,gff,sense = 'both',unique = 0,extension = 200,floor = 0,density = False,rpm = False,binSize = 25,clusterGram = None,matrix = None,raw = False,includeJxnReads = False):
    ...
    #use multiprocessing pool to speed up mapping    
    p = Pool(cpu_count())
    g = partial(mapFunc,bam=bam,newGFF=newGFF,MMR=MMR,sense=sense,unique=unique,extension=extension,floor=floor,density=density,rpm=rpm,binSize=binSize,clusterGram=clusterGram,matrix=matrix,raw=raw,includeJxnReads=includeJxnReads)
    chunks = [gff[i:i + 5000] for i in xrange(0, len(gff), int(len(gff)/cpu_count()))]
    newGFF = p.map(g, chunks)
    p.terminate()
    newGFF = list(itertools.chain.from_iterable(newGFF))
    return newGFF

def mapFunc(gff,bam,newGFF,MMR,sense = 'both',unique = 0,extension = 200,floor = 0,density = False,rpm = False,binSize = 25,clusterGram = None,matrix = None,raw = False,includeJxnReads = False):
