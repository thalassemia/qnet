...
outDir = system("echo $SCRATCH/tfchip/goodBedsSALL2/", intern = TRUE)
dir.create(outDir)
...
factors <- inner_join(index, de, by="Factor") %>% select(DCid, Factor, rank, Cell_line, Cell_type, Tissue_type, FastQC, UniquelyMappedRatio, PBC, PeaksFoldChangeAbove10, FRiP, PeaksUnionDHSRatio, log2FoldChange, padj)
