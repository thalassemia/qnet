...
# get rank of bed file (add 1000 to Cistrome ranks to always prefer ENCODE)
encodeKey = pd.read_csv(os.path.expandvars("$SCRATCH/cobinding/beds/encodeKey.csv"))
encodeKey.loc[:,"Path"] = [i.split("/")[-1] for i in encodeKey["Path"]]
def num(x):
    try:
        return int(x.split("/")[-1].split(".bed")[0]) + 1000
    except:
        if x.split("/")[-1] in encodeKey.loc[:,"Path"].values:
            return encodeKey.index[encodeKey["Path"] == x.split("/")[-1]].tolist()[0]
...
# create 11 cobinding maps using different sets of data each time if possible so there can be 11*10 = 110 combinations of training and test sets for 110 models
for count in range(11):
    ...
    cobindUU.to_csv(os.path.join(cobindDir, "UU_" + str(count) + ".csv"), sep = "\t")
    cobindUD.to_csv(os.path.join(cobindDir, "DD_" + str(count) + ".csv"), sep = "\t")
    cobindDD.to_csv(os.path.join(cobindDir, "DD_" + str(count) + ".csv"), sep = "\t")
    cobindDU.to_csv(os.path.join(cobindDir, "DD_" + str(count) + ".csv"), sep = "\t")
    cobindUp.to_csv(os.path.join(cobindDir, "up_" + str(count) + ".csv"), sep = "\t")
    cobindDown.to_csv(os.path.join(cobindDir, "down_" + str(count) + ".csv"), sep = "\t")
    cobind_Up.to_csv(os.path.join(cobindDir, "_up_" + str(count) + ".csv"), sep = "\t")
    cobind_Down.to_csv(os.path.join(cobindDir, "_down_" + str(count) + ".csv"), sep = "\t")
    cobindAll.to_csv(os.path.join(cobindDir, "all_" + str(count) + ".csv"), sep = "\t")
